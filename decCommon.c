...
static decFloat * decFinalize(decFloat *df, bcdnum *num,
  ...
  if (!NUMISSPECIAL(num)) {
    ...
    drop=MAXI(length-DECPMAX, DECQTINY-num->exponent); //r_ DECQTINY = 398,DECPMAX=16  for 64 bit //#define MAXI(x,y) ((x)<(y)?(y):(x))
  ...
  if (!NUMISSPECIAL(num)) {             // is still finite
    ...
    uInt code=(uexp>>DECECONL)<<4;      // top two bits of exp
										/* For example if exp =0;
										398 bais for 64 bit binary is
										110001110>>8
										000000001<<4
										000010000 = 16 in decimal will top two bit
										*/

    // [msd==0]
    // look up the combination field and make high word
    encode=DECCOMBFROM[code];           // indexed by (0-2)*16+msd
    encode|=(uexp<<(32-6-DECECONL)) & 0x03ffffff; // 8 exponent continuation
    }
   else encode=num->exponent;           // special [already in word]
  encode|=num->sign;                    //  a|=b and a = a | b same // add sign
  ...
  #elif DECPMAX==16 // in 64 bit ward has 2 item in 10 digit each 
    getDPDt(dpd, 4); encode|=dpd<<8;  //a|=b and a = a | b same 
  ...
  #elif DECPMAX==34 //// in 64 bit ward has 2 item in 10 digit each 
...
decFloat * decFloatFromString(decFloat *result, const char *string,
  ...
  const  char *c;                  // work
  uByte *ub;                       // ..-212404405515301E-151
  uInt   uiwork;                   // for macros
  bcdnum num;                      // collects data for finishing
  uInt   error=DEC_Conversion_syntax;      // assume the worst
  uByte  buffer[ROUNDUP(DECSTRING+11, 8)]; //FFS1- room for most coefficents for 64 bit it is 40,
  ...
  for(;;) {                             // once-only 'loop'
    ...
    for (c=string;; c++) {              // -> input character
      ...
      if (c==string) {                  // first in string...
        if (*c=='-') {                  // valid - sign
          cfirst++;
          num.sign=DECFLOAT_Sign; //r_#define DECFLOAT_Sign  0x80000000     /* 1 00000 00 Sign */ (-ve)
          continue;}
        if (*c=='+') {                  // valid + sign
          cfirst++;
          continue;}
    ...
    if (digits>0) {                     // had digits and/or dot
      ...
      if (*c!='\0') {                   // something follows the coefficient
        ...
        if (c>firstexp+DECEMAXD) exp=DECEMAX*2; //r_DECEMAXD =3 fro 64 bit 
      ...
      ub=buffer;
      if (digits<=(Int)(sizeof(buffer)-3)) { // r_ 37 for 64 bit//[-3 allows by-4s copy]
		...
		//r_#define UBFROMUI(b, i)  (uiwork=(i), memcpy(b, (void *)&uiwork, 4), uiwork)
		//r_ #define UBTOUI(b)  (memcpy((void *)&uiwork, b, 4), uiwork)
...
char * decFloatToString(const decFloat *df, char *string){
  ...
  #elif DECPMAX==16 ///5 for 64 bit count of declets highest biased exponent (Elimit-1) 
  dpd2char(sourhi>>8);                  // declet 1
  dpd2char((sourhi<<2) | (sourlo>>30)); // declet 2
  dpd2char(sourlo>>20);                 // declet 3
  dpd2char(sourlo>>10);                 // declet 4
  ...
  #elif DECPMAX==34//11 for 128 bit count of declets highest biased exponent (Elimit-1) 
  dpd2char(sourhi>>4);                  // declet 1
  dpd2char((sourhi<<6) | (sourmh>>26)); // declet 2
  dpd2char(sourmh>>16);                 // declet 3
  dpd2char(sourmh>>6);                  // declet 4
